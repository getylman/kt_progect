...
PowerUp::PowerUp(const float i_x, const float i_y) :
	dead(0),
	...
	start_y(i_y)
{
	texture.loadFromFile("Resources/Images/PowerUp.png");
}

bool PowerUp::get_dead() const
{
	return dead;
}

void PowerUp::draw(const unsigned i_view_x, sf::RenderWindow& i_window)
{
...
void PowerUp::set_dead(const bool i_value)
{
	dead = i_value;
}
	...
	if (0 == dead)
	{
		if (-CELL_SIZE < y && x >= static_cast<int>(i_view_x) - CELL_SIZE - ENTITY_UPDATE_AREA && x < ENTITY_UPDATE_AREA + SCREEN_WIDTH + i_view_x && y < SCREEN_HEIGHT)
		{
			...
			else
			{
				...
				horizontal_hit_box = { POWERUP_SPEED * horizontal_direction + x, y, CELL_SIZE, CELL_SIZE };
					...
					}))
				{
					...
					{
						x = CELL_SIZE * (ceil((POWERUP_SPEED * horizontal_direction + x) / CELL_SIZE) - 1);
					}
					else
					{
						x = CELL_SIZE * (1 + floor((POWERUP_SPEED * horizontal_direction + x) / CELL_SIZE));
					}
...
sf::FloatRect PowerUp::get_hit_box() const
{
