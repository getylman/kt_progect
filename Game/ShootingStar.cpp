...
ShootingStar::ShootingStar(const bool i_underground, const float i_x, const float i_y) :
	Enemy(i_x, i_y),
	check_collision(1),
	flipped(1),
	...
	state(0),
	get_out_timer(SHOOTINGSTAR_REINCARNATION_DURATION),
	get_out_animation(CELL_SIZE, "Resources/Images/KoopaGetOut.png", SHOOTINGSTAR_REINCARNETING_SPEED),
	walk_animation(CELL_SIZE, "Resources/Images/KoopaWalk.png", SHOOTINGSTAR_WALK_ANIMATION_SPEED)
{
	horizontal_speed = -SHOOTINGSTAR_SPEED;
...
bool ShootingStar::get_dead(const bool i_deletion) const
{
	if (1 == i_deletion)
	{
		return dead;
...
void ShootingStar::die(const unsigned char i_death_type)
{
	switch (i_death_type)
	{
	case 0:
	...
	case 2:
	{
		//Dying from ShootingStar's shell.
...
void ShootingStar::draw(const unsigned i_view_x, sf::RenderWindow& i_window)
{
	if (-CELL_SIZE < round(y) && round(x) > static_cast<int>(i_view_x) - CELL_SIZE && round(x) < SCREEN_WIDTH + i_view_x && round(y) < SCREEN_HEIGHT)
	{
		...
		{
			if (0 == get_dead(0) && 1 == state && get_out_timer <= 0.25f * SHOOTINGSTAR_REINCARNATION_DURATION)
			{
...
void ShootingStar::update(const unsigned i_view_x, const std::vector<std::shared_ptr<Enemy>>& i_enemies, const MapManager& i_map_manager, Hero& i_mario)
{
	//I've already explained most of the code here in the Hero and OrdStar classes.
	...
	if (-CELL_SIZE < y && x >= static_cast<int>(i_view_x) - CELL_SIZE - ENTITY_UPDATE_AREA && x < ENTITY_UPDATE_AREA + SCREEN_WIDTH + i_view_x && y < SCREEN_HEIGHT)
	{
		...
		if (0 == get_dead(0))
		{
			...
			if (0 == i_mario.get_dead() && 1 == get_hit_box().intersects(i_mario.get_hit_box()))
			{
				if (1 == check_collision)
				{
					if (0 == state)
					{
						...
						{
							//We check the collision only once after we collide with Hero.
							check_collision = 0;
					...
					else if (1 == state)
					{
						...
						//The direction is based on where Hero kicked ShootingStar.
						if (x < i_mario.get_x())
						{
							horizontal_speed = -NEUTRON_SPEED;
						}
						else
						{
							horizontal_speed = NEUTRON_SPEED;
			...
			{
				//Once Hero stops intersecting with ShootingStar, start checking for collisions again.
				check_collision = 1;
			}
			...
			//ShootingStar can get out of his shell here.
			if (1 == state)
			{
				get_out_timer--;
