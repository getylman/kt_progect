...
{
	//We'll use this to make sure that we check the collision with Hero only once when he collides with us.
	bool check_collision;
	bool flipped;
	//Look at the declaration of the OrdStar class. I don't wanna explain this again.
	...
	//0 - ShootingStar walk
	//1 - ShootingStar shell
	//2 - ShootingStar shell slide
	unsigned char state;
...
public:
	ShootingStar(const bool i_underground, const float i_x, const float i_y);
	...
	void update(const unsigned i_view_x, const std::vector<std::shared_ptr<Enemy>>& i_enemies, const MapManager& i_map_manager, Hero& i_mario);
};